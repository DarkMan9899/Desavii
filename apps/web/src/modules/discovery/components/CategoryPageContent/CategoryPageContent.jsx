...
export default function CategoryPageContent() {
  ...
  // Step 2.4 (Villa TOP background only) — same structural rule again,
  // via its own dedicated classes/branch, never touching Car Rental's,
  // Hotel's, or Apartment's.
  const isVillaTop = category.slug === 'villas';
  // Step 2.5 (Guest House TOP background only) — same structural rule
  // again, via its own dedicated classes/branch, never touching Car
  // Rental's, Hotel's, Apartment's, or Villa's.
  const isGuestHouseTop = category.slug === 'guest-houses';
  // Step 2.6 (Restaurant TOP background only) — same structural rule
  // again, via its own dedicated classes/branch, never touching Car
  // Rental's, Hotel's, Apartment's, Villa's, or Guest House's.
  const isRestaurantTop = category.slug === 'restaurants';
  // Step 2.7 (Tours TOP background only) — same structural rule again,
  // via its own dedicated classes/branch, never touching Car Rental's,
  // Hotel's, Apartment's, Villa's, Guest House's, or Restaurant's.
  const isToursTop = category.slug === 'tours';
  // Step 2.8 (Attractions TOP background only) — same structural rule
  // again, via its own dedicated classes/branch, never touching Car
  // Rental's, Hotel's, Apartment's, Villa's, Guest House's,
  // Restaurant's, or Tours'.
  const isAttractionsTop = category.slug === 'attractions';
  // Step 2.9 (Entertainment TOP background only, the last of the 9
  // categories) — same structural rule again, via its own dedicated
  // classes/branch, never touching any of the other 8 categories'.
  const isEntertainmentTop = category.slug === 'entertainment-venues';

  // A plain function (not a nested ternary — eslint's `no-nested-ternary`
  // rejects a chain of more than 2 branches) covering the 9 possible
  // TOP-section layouts: each of the 9 real categories' own dedicated
  // stage (each its own classes, never shared/renamed) — the shared
  // generic treatment below is now unreachable for any real category
  ...
  function renderTopSectionContent() {
    ...
    if (isVillaTop) {
      return (
        <>
          {/* Step 2.4 — same structural rule again: the heading lives
              OUTSIDE the dark "stage" below, on the page's own normal
              background. */}
          <span className={styles.topSectionEyebrow} aria-hidden="true">
            {t('advertising.topBadge')}
          ...
          <h2
            id="category-top-heading"
            className={styles.topSectionHeadingVilla}
          >
            {t('discovery.category.topHeading', { category: category.name })}
          </h2>
          {/* The "stage" — VillaTopBackground's dusk mountain/retreat
              environment and the carousel, together, own dark chrome now
              separate from the heading above. Same canonical `topSlide`
              width every other category's TOP carousel uses — no
              category-specific card/slide geometry here either. */}
          <div className={styles.villaStage}>
            <CategoryTopBackground categorySlug={category.slug} />
            <Showcase
              ariaLabel={t('discovery.category.topHeading', {
                category: category.name,
    ...
    if (isGuestHouseTop) {
      return (
        <>
          {/* Step 2.5 — same structural rule again: the heading lives
              OUTSIDE the dark "stage" below, on the page's own normal
              background. */}
          <span className={styles.topSectionEyebrow} aria-hidden="true">
            {t('advertising.topBadge')}
          ...
          <h2
            id="category-top-heading"
            className={styles.topSectionHeadingGuestHouse}
          >
            {t('discovery.category.topHeading', { category: category.name })}
          </h2>
          {/* The "stage" — GuestHouseTopBackground's warm village-house
              environment and the carousel, together, own dark chrome now
              separate from the heading above. Same canonical `topSlide`
              width every other category's TOP carousel uses — no
              category-specific card/slide geometry here either. */}
          <div className={styles.guestHouseStage}>
            <CategoryTopBackground categorySlug={category.slug} />
            <Showcase
              ariaLabel={t('discovery.category.topHeading', {
                category: category.name,
    ...
    if (isRestaurantTop) {
      return (
        <>
          {/* Step 2.6 — same structural rule again: the heading lives
              OUTSIDE the dark "stage" below, on the page's own normal
              background. */}
          <span className={styles.topSectionEyebrow} aria-hidden="true">
            {t('advertising.topBadge')}
          ...
          <h2
            id="category-top-heading"
            className={styles.topSectionHeadingRestaurant}
          >
            {t('discovery.category.topHeading', { category: category.name })}
          </h2>
          {/* The "stage" — RestaurantTopBackground's table-setting
              environment and the carousel, together, own dark chrome now
              separate from the heading above. Same canonical `topSlide`
              width every other category's TOP carousel uses — no
              category-specific card/slide geometry here either. */}
          <div className={styles.restaurantStage}>
            <CategoryTopBackground categorySlug={category.slug} />
            <Showcase
              ariaLabel={t('discovery.category.topHeading', {
                category: category.name,
    ...
    if (isToursTop) {
      return (
        <>
          {/* Step 2.7 — same structural rule again: the heading lives
              OUTSIDE the dark "stage" below, on the page's own normal
              background. */}
          <span className={styles.topSectionEyebrow} aria-hidden="true">
            {t('advertising.topBadge')}
          ...
          <h2
            id="category-top-heading"
            className={styles.topSectionHeadingTours}
          >
            {t('discovery.category.topHeading', { category: category.name })}
          </h2>
          {/* The "stage" — ToursTopBackground's topographic-map/trail
              environment and the carousel, together, own dark chrome now
              separate from the heading above. Same canonical `topSlide`
              width every other category's TOP carousel uses — no
              category-specific card/slide geometry here either. */}
          <div className={styles.toursStage}>
            <CategoryTopBackground categorySlug={category.slug} />
            <Showcase
              ariaLabel={t('discovery.category.topHeading', {
                category: category.name,
    ...
    if (isAttractionsTop) {
      return (
        <>
          {/* Step 2.8 — same structural rule again: the heading lives
              OUTSIDE the dark "stage" below, on the page's own normal
              background. */}
          <span className={styles.topSectionEyebrow} aria-hidden="true">
            {t('advertising.topBadge')}
          ...
          <h2
            id="category-top-heading"
            className={styles.topSectionHeadingAttractions}
          >
            {t('discovery.category.topHeading', { category: category.name })}
          </h2>
          {/* The "stage" — AttractionsTopBackground's heritage-monument/
              editorial environment and the carousel, together, own dark
              chrome now separate from the heading above. Same canonical
              `topSlide` width every other category's TOP carousel uses —
              no category-specific card/slide geometry here either. */}
          <div className={styles.attractionsStage}>
            <CategoryTopBackground categorySlug={category.slug} />
            <Showcase
              ariaLabel={t('discovery.category.topHeading', {
                category: category.name,
    ...
    if (isEntertainmentTop) {
      return (
        <>
          {/* Step 2.9 — same structural rule again: the heading lives
              OUTSIDE the dark "stage" below, on the page's own normal
              background. */}
          <span className={styles.topSectionEyebrow} aria-hidden="true">
            {t('advertising.topBadge')}
          ...
          <h2
            id="category-top-heading"
            className={styles.topSectionHeadingEntertainment}
          >
            {t('discovery.category.topHeading', { category: category.name })}
          </h2>
          {/* The "stage" — EntertainmentTopBackground's stage/spotlight
              environment and the carousel, together, own dark chrome now
              separate from the heading above. Same canonical `topSlide`
              width every other category's TOP carousel uses — no
              category-specific card/slide geometry here either. */}
          <div className={styles.entertainmentStage}>
            <CategoryTopBackground categorySlug={category.slug} />
            <Showcase
              ariaLabel={t('discovery.category.topHeading', {
                category: category.name,
      ...
      {!isTopPending && topListings?.length > 0 && (
        <section
          className={[
            ...
            isVillaTop && styles.topSectionVilla,
            isGuestHouseTop && styles.topSectionGuestHouse,
            isRestaurantTop && styles.topSectionRestaurant,
            isToursTop && styles.topSectionTours,
            isAttractionsTop && styles.topSectionAttractions,
            isEntertainmentTop && styles.topSectionEntertainment,
          ]
            .filter(Boolean)
            .join(' ')}
